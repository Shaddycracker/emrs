...
export const getAllGalleryImages = async (): Promise<GalleryImage[]> => {
    const galleryQuery = query(collection(db, "gallery-photo"), orderBy("uploadedAt", "desc"))
...
export const getGalleryImages = async (count = 10): Promise<(GalleryImage[])> => {
    const galleryQuery = query(collection(db, "gallery-photo"), orderBy("uploadedAt", "desc"), limit(count))
    const querySnapshot = await getDocs(galleryQuery)
    return querySnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data()
...
export const addGalleryImage = async (imageData: GalleryImage): Promise<string> => {
    const docRef = await addDoc(collection(db, "gallery-photo"), imageData)
    return docRef.id
}

export const deleteGalleryImage = async (id: string): Promise<boolean> => {
    const imageRef = doc(db, "gallery-photo", id)
